...
private:
    ...
    // dentro da caixa FILTER agora: EQ ON/OFF (liga/desliga o EQ inteiro
    // que atua antes do compressor, parâmetro real via attachment). O
    // HPF do detector do compressor foi removido da UI (redundante - o
    // Pre EQ já tem seu próprio Low Cut).
    HardwareButton preOnButton { "EQ ON/OFF" };
